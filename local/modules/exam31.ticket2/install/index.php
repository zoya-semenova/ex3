...
class exam31_ticket2 extends CModule
{
	var $MODULE_ID = 'exam31.ticket2';
	...
	public function __construct()
	{
		...
		$this->filesPath = [
			...
			'ACTIVITIES' => '/local/activities/examticketactivity2'
		];
	...
	public function InstallDB(): bool
	{
		...
		$entity = SomeElement2Table::getEntity();
		$tableName = SomeElement2Table::getTableName();
	...
	{
		copyDirFiles(
			$this->fileRoot . '/local/modules/' . $this->MODULE_ID . '/install/activities/examticketactivity2',
			$this->fileRoot . '/local/activities/examticketactivity2',
			true,
			true
		);
		copyDirFiles(
		...
		copyDirFiles(
			$this->fileRoot . '/local/modules/' . $this->MODULE_ID . '/install/public/exam312',
			$this->fileRoot . '/exam312',
			true,
			true
		);
	}
	...
	{
		Directory::deleteDirectory($this->fileRoot . '/local/activities/examticketactivity2');
		Directory::deleteDirectory($this->fileRoot . '/local/components/' . $this->MODULE_ID);
		Directory::deleteDirectory($this->fileRoot . '/exam312');
	}
	...
	public function InstallUrlRewriterRuls(): void
	{
		UrlRewriter::add('s1',
			[
				'ID' => 'exam31.ticket2:examelements',
				'CONDITION' => '#^/exam312/#',
				'PATH' => '/exam312/index.php',
			]
		);
	}
